...
import Tetrominos.OBlock;
import Tetrominos.ZBlock;
import Tetrominos.SBlock;
import Tetrominos.JBlock;
...
public class CGameGrid {
    ...
    private CPanelGame panel;

    public CGameGrid() {
    }

    public CGameGrid(CPanelGame panel, int row, int column) {
        this.panel = panel;
        this.column = column;
        this.row = row;
    ...
    public CGameGrid(int row, int column) {
        this.column = column;
        this.row = row;
        grid = new int[row][column];
        // Initialisation de chaque élément à 1
...
if (index_rotation >= 0 && index_rotation < currentPiece.getRotationCount()) {
 ...
 grid's bounds.
    ...
    public CGameGrid clone() {
        CGameGrid clonedGrid = new CGameGrid(grid[0].length, grid.length);
...
//        index_rotation = 0;
        ...
        if (panel instanceof CPanelGameOpenAI) {
            CPanelGameOpenAI panelAI = (CPanelGameOpenAI) panel;
